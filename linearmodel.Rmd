...
* Download [linearmodels.zip](https://github.com/uvastatlab/phdplus/raw/master/linearmodels.zip). It contains the R script we'll use today (linearmodel.R), an R script to recreate the introductory example here (intro_model.R), and a project file. 
...
For every unit change in square feet bins -- in this case, a 250 increase -- we expect improvement value to increase by `r round(lm_impvalue$coefficients[[2]], 0)` dollars. The intercept, in this case, is essentially meaningless -- why?
...
In prose, errors are independently and identically distributed (iid) around a mean of 0 with a constant variance ($\sigma^2$) and drawn from a normal distribution.
