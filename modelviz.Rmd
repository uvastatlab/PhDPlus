...
- Download [modelvizR.zip](https://github.com/uvastatlab/phdplus/raw/master/datavizR.zip). It contains an R script (dat_viz.R) and the content of this page as a slide deck (vmcr-pres.pdf).
...
`ggeffects` is a younger package, currently at version 0.8. The `effects` package is older, currently at version 4.0. 
...
The `Effect` function returns a list that contains an object called `x`. The `x` object is a data frame contains the focal predictor values.
